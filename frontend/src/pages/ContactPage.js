...
export default function ContactPage() {
  ...
  const inputStyle = (k) => ({
    width: '100%',
    background: focused === k ? 'rgba(0, 212, 170, 0.04)' : 'rgba(255,255,255,0.03)',
    border: `1px solid ${focused === k ? 'rgba(0, 212, 170, 0.5)' : 'rgba(255,255,255,0.09)'}`,
    borderRadius: 10,
    padding: '11px 14px',
    color: '#fff',
    fontSize: 14,
    ...
    boxShadow: focused === k ? '0 0 0 3px rgba(0, 212, 170, 0.08)' : 'none',
  });
  ...
  return (
    <>
      <style>{`
        ...
        .trust-card:hover  { border-color: rgba(0, 212, 170, 0.25) !important; transform: translateY(-2px); }
        .submit-btn:hover:not(:disabled) { background: #00a88a !important; transform: translateY(-1px); box-shadow: 0 8px 24px rgba(0, 212, 170, 0.25) !important; }
      ...
      <div style={{ minHeight:'100vh', padding:'clamp(70px,10vw,90px) 0 0', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'fixed', top:0, left:0, right:0, bottom:0, pointerEvents:'none', zIndex:0, overflow:'hidden' }}>
          <div style={{ position:'absolute', top:0, left:0, right:0, height:2, background:'linear-gradient(90deg,transparent,rgba(0, 212, 170, 0.06),transparent)', animation:'scan 8s linear infinite' }} />
        </div>

        {/* bg glow blobs */}
        <div style={{ position:'fixed', top:'15%', left:'-8%', width:400, height:400, borderRadius:'50%', background:'radial-gradient(circle,rgba(0, 212, 170, 0.055) 0%,transparent 70%)', pointerEvents:'none', zIndex:0 }} />
        <div style={{ position:'fixed', bottom:'10%', right:'-5%', width:340, height:340, borderRadius:'50%', background:'radial-gradient(circle,rgba(0, 212, 170, 0.04) 0%,transparent 70%)', pointerEvents:'none', zIndex:0 }} />
        ...
        <div style={{ position:'relative', zIndex:1, maxWidth:1160, margin:'0 auto', padding:'0 clamp(16px,5%,5%)' }}>
          ...
          <section style={{ position: 'relative', overflow: 'hidden', borderRadius: 20, marginBottom: 56, padding: '56px 20px' }}>
            ...
            <div style={{ position: 'relative', zIndex: 2, textAlign:'center', marginBottom:'clamp(36px,6vw,64px)' }}>
              <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'5px 14px', borderRadius:999, border:'1px solid rgba(0, 212, 170, 0.25)', background:'rgba(0, 212, 170, 0.06)', marginBottom:18 }}>
                <span style={{ width:7, height:7, borderRadius:'50%', background:'#00d4aa', display:'inline-block', animation:'blink 1.4s ease-in-out infinite' }} />
                <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:10, color:'#00d4aa', letterSpacing:2, textTransform:'uppercase', fontWeight:700 }}>Secure Channel Open</span>
              </div>
              <h1 style={{ fontFamily:"'Poppins',sans-serif", fontSize:'clamp(32px,5.5vw,64px)', fontWeight:900, color:'#fff', letterSpacing:-2, lineHeight:1.05, margin:'0 0 16px' }}>
                Let's Build<br />
                <span style={{ WebkitTextStroke:'1px rgba(0, 212, 170, 0.6)', color:'transparent' }}>Something</span>
                {' '}<span style={{ color:'#00d4aa' }}>Real</span>
              </h1>
              <p style={{ color:'rgba(255,255,255,0.38)', fontSize:15, maxWidth:400, margin:'0 auto', lineHeight:1.8, fontFamily:"'Inter',sans-serif" }}>
          ...
          <div className="contact-main-grid">
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
              ...
              <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:20, overflow:'hidden' }}>
                <div style={{ padding:'12px 18px', background:'rgba(255,255,255,0.03)', borderBottom:'1px solid rgba(255,255,255,0.06)', display:'flex', alignItems:'center', gap:8 }}>
                  <div style={{ display:'flex', gap:5 }}>
                    {['#EF4444','#F59E0B','#00d4aa'].map((c,i) => <div key={i} style={{ width:10, height:10, borderRadius:'50%', background:c, opacity:0.7 }} />)}
                  </div>
                  <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:10, color:'rgba(255,255,255,0.2)', letterSpacing:1 }}>axentralab — contact.sh</span>
                </div>
                <div style={{ padding:'20px 18px', fontFamily:"'Space Mono',monospace", fontSize:12, lineHeight:2 }}>
                  {[
                    { prompt:'>', cmd:'init contact_session', color:'#00d4aa' },
                    ...
                    { prompt:'>', cmd:'status: ready_to_build ▋', color:'#00d4aa' },
                  ].map((l,i) => (
                    <div key={i} style={{ display:'flex', gap:10, animation:`fadeUp 0.4s ease ${i*0.1}s both` }}>
                      <span style={{ color:'rgba(0, 212, 170, 0.5)' }}>{l.prompt}</span>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                {[
                  { icon:'📧', label:'Email',    val:'axentralab@gmail.com',       color:'#00d4aa' },
            ...
            <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:24, overflow:'hidden' }}>
              ...
              <div style={{ padding:'16px 28px', background:'rgba(255,255,255,0.025)', borderBottom:'1px solid rgba(255,255,255,0.06)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                  <span style={{ width:7, height:7, borderRadius:'50%', background:'#00d4aa', display:'inline-block', animation:'blink 2s ease-in-out infinite' }} />
              ...
              <div style={{ padding:'clamp(18px,4vw,28px)' }}>
                {submitted ? (
                  <div style={{ textAlign:'center', padding:'48px 0', animation:'fadeUp 0.5s ease' }}>
                    <div style={{ width:72, height:72, borderRadius:'50%', background:'rgba(0, 212, 170, 0.1)', border:'2px solid rgba(0, 212, 170, 0.3)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:32, margin:'0 auto 20px' }}>✅</div>
                    <h2 style={{ fontFamily:"'Poppins',sans-serif", color:'#00d4aa', fontSize:22, fontWeight:900, marginBottom:10, letterSpacing:-0.5 }}>Message Received!</h2>
                    ...
                    <div style={{ marginTop:28, padding:'12px 20px', background:'rgba(0, 212, 170, 0.05)', border:'1px solid rgba(0, 212, 170, 0.15)', borderRadius:12, display:'inline-block' }}>
                      <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:11, color:'rgba(0, 212, 170, 0.7)' }}>ref_id: AXL-{Math.floor(Math.random()*90000+10000)}</span>
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:16 }}>
                      ...
                      <button type="submit" disabled={loading} className="submit-btn"
                        style={{ padding:'14px', background:'#00d4aa', color:'#0f172a', border:'none', borderRadius:12, fontSize:15, fontWeight:800, fontFamily:"'Poppins',sans-serif", cursor:'pointer', transition:'all 0.2s', display:'flex', alignItems:'center', justifyContent:'center', gap:10, letterSpacing:-0.3 }}>
                        {loading
