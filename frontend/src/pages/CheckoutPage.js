  ...
  return (
    <>
      <style>{`
        ...
        .co-field { background: var(--bg2); border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; padding: 10px 14px; font-size: 14px; color: #fff; }
        .spinner { width: 16px; height: 16px; border: 2px solid var(--border); border-top-color: #fff; border-radius: 50%; animation: spin 0.7s linear infinite; display: inline-block; }
      ...
      <div style={{ padding: '100px 5% 80px', minHeight: '100vh', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 860, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <h1 style={{ fontFamily: "var(--font-h)", fontSize: 'clamp(26px,4vw,44px)', fontWeight: 900, color: 'var(--text)', letterSpacing: -1, marginBottom: 8 }}>Checkout</h1>
          <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 13, marginBottom: 36, fontFamily: "var(--font-m)" }}>Review your order and complete payment securely.</p>
          ...
          <div className="checkout-grid">
            ...
            <div>
              ...
              <div className="co-section" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: 28, marginBottom: 20, overflow: 'hidden' }}>
                ...
                <h3 style={{ fontFamily: "var(--font-h)", fontSize: 15, fontWeight: 700, color: 'var(--text)', marginBottom: 20 }}>👤 Customer Details</h3>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  {[{ label: 'Name', val: user?.name }, { label: 'Email', val: user?.email }, { label: 'Company', val: user?.company || '—' }].map(f => (
                    <div key={f.label} style={{ gridColumn: f.label === 'Email' ? 'span 2' : 'auto' }}>
                      <div style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.35)', marginBottom: 5, fontFamily: "var(--font-m)", letterSpacing: 0.5, textTransform: 'uppercase' }}>{f.label}</div>
              ...
              <div className="co-section" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: 28, marginBottom: 20, overflow: 'hidden' }}>
                ...
                <h3 style={{ fontFamily: "var(--font-h)", fontSize: 15, fontWeight: 700, color: 'var(--text)', marginBottom: 20 }}>📦 Order Items</h3>
                {cart.map((item, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: i < cart.length - 1 ? '1px solid var(--bg2)' : 'none', gap: 12, flexWrap: 'wrap' }}>
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--text)' }}>{item.serviceTitle}</div>
                      <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 3, fontFamily: "var(--font-m)" }}>
                        {item.plan} · {item.billing === 'monthly' ? 'Monthly' : item.billing === 'yearly' ? 'Yearly' : 'One-time'} · ×{item.quantity || 1}
                      </div>
                    </div>
                    <span style={{ fontFamily: "var(--font-h)", fontWeight: 800, color: PRIMARY2, fontSize: 16, flexShrink: 0 }}>
              ...
              <div className="co-section" style={{ background: `${PRIMARY}06`, border: `1px solid ${PRIMARY}20`, borderRadius: 14, padding: '16px 20px', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                ...
                <div>
                  <div style={{ fontFamily: "var(--font-h)", fontWeight: 700, fontSize: 13, color: 'var(--text)', marginBottom: 4 }}>
            ...
            <div>
              <div className="checkout-summary-sticky" style={{ background: 'rgba(255,255,255,0.025)', border: `1px solid ${PRIMARY}20`, borderRadius: 18, padding: 28, overflow: 'hidden' }}>
                <div style={{ height: 3, background: `linear-gradient(90deg,${PRIMARY},${ACCENT})`, margin: '-28px -28px 24px' }} />
                <h3 style={{ fontFamily: "var(--font-h)", fontSize: 15, fontWeight: 700, color: 'var(--text)', marginBottom: 20 }}>💳 Payment Summary</h3>

                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10, fontSize: 13, color: 'var(--muted)' }}>
                  <span>Subtotal ({cart.length} item{cart.length > 1 ? 's' : ''})</span>
                  <span>${total.toLocaleString()}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20, fontSize: 13, color: 'var(--muted)' }}>
                  <span>Tax</span><span style={{ fontSize: 11 }}>At checkout</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '16px 0', borderTop: '1px solid rgba(255,255,255,0.07)', fontFamily: "var(--font-h)", fontSize: 20, fontWeight: 900, color: 'var(--text)' }}>
                  <span>Total</span>
                ...
                <button onClick={handleCheckout} disabled={loading}
                  style={{ width: '100%', padding: '14px', fontSize: 15, background: loading ? 'rgba(255,255,255,0.08)' : `linear-gradient(135deg,${PRIMARY},${ACCENT})`, border: 'none', borderRadius: 12, color: 'var(--text)', fontFamily: "var(--font-h)", fontWeight: 800, cursor: loading ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s', boxShadow: loading ? 'none' : `0 4px 18px ${PRIMARY}40` }}
                ...
                <button onClick={() => setContactModalOpen(true)}
                  style={{ width: '100%', padding: '14px', fontSize: 15, marginTop: 12, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, color: 'var(--text)', fontFamily: "var(--font-h)", fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s' }}
                ...
                <div style={{ marginTop: 16, padding: 14, background: `${PRIMARY}05`, border: `1px solid ${PRIMARY}12`, borderRadius: 10 }}>
                  <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', lineHeight: 1.7, fontFamily: "var(--font-m)", margin: 0 }}>
