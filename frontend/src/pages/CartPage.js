...
export default function CartPage() {
  ...
  if (count === 0) return (
    <div style={{ minHeight: '80vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '80px 5%' }}>
      ...
      <h2 style={{ fontFamily: "var(--font-h)", fontSize: 26, fontWeight: 800, color: 'var(--text)', marginBottom: 12 }}>Your cart is empty</h2>
    ...
    <>
      ...
      <div style={{ padding: '100px 5% 80px', minHeight: '100vh', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <h1 style={{ fontFamily: "var(--font-h)", fontSize: 'clamp(26px,4vw,44px)', fontWeight: 900, color: 'var(--text)', letterSpacing: -1, marginBottom: 8 }}>Your Cart</h1>
          <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 14, marginBottom: 36, fontFamily: "var(--font-m)" }}>{count} item{count !== 1 ? 's' : ''} · ${total.toLocaleString()} total</p>
          ...
          <div className="cart-grid">
            ...
            <div>
              {cart.map((item, i) => (
                <div key={i} className="cart-item cart-item-row"
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: "var(--font-h)", fontWeight: 700, fontSize: 15, color: 'var(--text)', marginBottom: 5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.serviceTitle}</div>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 11, padding: '2px 10px', borderRadius: 6, background: `${PRIMARY}15`, border: `1px solid ${PRIMARY}30`, color: PRIMARY2, fontFamily: "var(--font-m)" }}>{item.plan}</span>
                      <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', padding: '2px 8px', background: 'var(--bg2)', borderRadius: 6, fontFamily: "var(--font-m)" }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <button onClick={() => updateQuantity(item.serviceId, item.plan, -1)} className="cart-qty-btn"
                      style={{ width: 30, height: 30, borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: '1px solid var(--border)', color: 'var(--text)', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
                    <span style={{ fontFamily: "var(--font-h)", fontWeight: 700, fontSize: 14, color: 'var(--text)', minWidth: 22, textAlign: 'center' }}>{item.quantity || 1}</span>
                    <button onClick={() => updateQuantity(item.serviceId, item.plan, 1)} className="cart-qty-btn"
                      style={{ width: 30, height: 30, borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: '1px solid var(--border)', color: 'var(--text)', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
                  </div>

                  <div style={{ fontFamily: "var(--font-h)", fontWeight: 900, fontSize: 18, color: '#A78BFA', minWidth: 70, textAlign: 'right' }}>
              ...
              <button onClick={handleClear}
                style={{ marginTop: 8, fontSize: 12, color: 'rgba(255,255,255,0.25)', background: 'none', border: 'none', textDecoration: 'underline', cursor: 'pointer', fontFamily: "var(--font-m)" }}>
                Clear cart
              </button>
            </div>
            ...
            <div>
              <div className="cart-summary-sticky" style={{ background: 'rgba(255,255,255,0.025)', border: `1px solid ${PRIMARY}20`, borderRadius: 20, padding: 28, position: 'sticky', top: 80, overflow: 'hidden' }}>
                ...
                <h3 style={{ fontFamily: "var(--font-h)", fontSize: 17, fontWeight: 800, color: 'var(--text)', marginBottom: 20 }}>Order Summary</h3>
                {cart.map((item, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--bg2)', fontSize: 13, color: 'rgba(255,255,255,0.55)' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '16px 0', fontSize: 20, fontWeight: 900, fontFamily: "var(--font-h)", color: 'var(--text)', borderTop: '1px solid rgba(255,255,255,0.08)', marginTop: 8 }}>
                  <span>Total</span>
                ...
                {isAuthenticated ? (
                  <button onClick={() => navigate('/checkout')} className="cart-checkout-btn"
                    style={{ width: '100%', padding: '14px', fontSize: 15, background: `linear-gradient(135deg,${PRIMARY},${ACCENT})`, border: 'none', borderRadius: 12, color: 'var(--text)', fontFamily: "var(--font-h)", fontWeight: 800, cursor: 'pointer', boxShadow: `0 4px 18px ${PRIMARY}40`, marginTop: 4 }}>
                    Proceed to Checkout →
                  </button>
                ) : (
                  <div>
                    <button onClick={() => navigate('/login', { state: { from: '/checkout' } })} className="cart-checkout-btn"
                      style={{ width: '100%', padding: '14px', fontSize: 14, background: `linear-gradient(135deg,${PRIMARY},${ACCENT})`, border: 'none', borderRadius: 12, color: 'var(--text)', fontFamily: "var(--font-h)", fontWeight: 800, cursor: 'pointer', boxShadow: `0 4px 18px ${PRIMARY}40`, marginBottom: 10, marginTop: 4 }}>
                      Login to Checkout →
                    </button>
                    <button onClick={() => navigate('/register')}
                      style={{ width: '100%', padding: '12px', fontSize: 13, background: 'transparent', border: `1px solid ${PRIMARY}35`, borderRadius: 12, color: PRIMARY2, fontFamily: "var(--font-h)", fontWeight: 700, cursor: 'pointer', transition: 'all 0.18s' }}
                ...
                <button onClick={() => setContactModalOpen(true)}
                  style={{ width: '100%', padding: '12px', fontSize: 13, marginTop: 12, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, color: 'var(--text)', fontFamily: "var(--font-h)", fontWeight: 700, cursor: 'pointer', transition: 'all 0.18s' }}
                ...
                <div style={{ marginTop: 20, padding: 14, background: `${PRIMARY}06`, border: `1px solid ${PRIMARY}15`, borderRadius: 12 }}>
                  <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)', lineHeight: 1.7, fontFamily: "var(--font-m)" }}>
